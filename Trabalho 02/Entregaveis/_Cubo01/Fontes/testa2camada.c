...
#define		RESOLVE_2_DIREITA_INVERSO_CMD	 "=resolve2direitaInverso" // cubo 5 //ok
#define     RESOLVE_2_RESOLVIDA_INVERSO_CMD	 "=resolve2resolvidaInverso" //cubo 6 //ok
#define		RESOLVE_CASO_PRESO_INVERSO_CMD	 "=resolve2CasoPresoInverso" // cubo 7
...
int populaCubo(int *config, int numCubo) {
	...
	else if (numCubo == 7){    //  Cubo inverso 2 peca presa
   ...
   TST_tpCondRet TST_EfetuarComando( char * ComandoTeste )
   {
		...
		/* Testar resolver 2 camada caso esquerda*/
			...
			if ( strcmp( ComandoTeste , RESOLVE_2_RESOLVIDA_INVERSO_CMD	) == 0 )
         {
			...
			/* Testar resolver caso preso*/
	
			if ( strcmp( ComandoTeste , RESOLVE_CASO_PRESO_INVERSO_CMD) == 0 )
         {
			 populaCubo(config, 7);
			CUB_CriarCUBO(&cubo, config);
         ...
         } /* fim ativa: Testar resolver caso preso */
